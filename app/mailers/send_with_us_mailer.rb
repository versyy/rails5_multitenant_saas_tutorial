class SendWithUsMailer < Devise::Mailer
  ...
  def sendwithus_client
    @sendwithus_client ||= Clients::SendWithUs.client
  end

  def deliver(template_id, address, payload)
    sendwithus_client.deliver_msg(
      template_id: template_id,
      to: { address: address },
      payload: payload
    )
  rescue Clients::SendWithUs::Error => e
